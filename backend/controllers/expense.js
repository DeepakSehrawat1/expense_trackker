...
const downloadExpense = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ fileURL: "", success: false, error: "Internal Server Error" });
  }
};
...
const addExpense = async (req, res) => {
  ...
  try {
    ...
    if (expenseamount === undefined || expenseamount <= 0) {
      ...
      return res
        .status(400)
        .json({ err: "Expense Amount is Missing or Invalid" });
    }
    ...
    if (!description || !category) {
      ...
      return res
        .status(400)
        .json({ err: "Empty Parameters for Description or Category" });
    }
